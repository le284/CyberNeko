...
```text
CyberNeko/
├── cat/                       # 5 组默认宠物资源；每组 1 招手、2 歪头、3 伸懒腰、4 眨眼睛
...
│   └── src/main.js            # 150ms 动画循环、宠物动作图片、设置、图片上传和气泡文案
...
- `cat/` 下的 5 组默认宠物图片会按宠物槽位自动使用；每组编号 `1` 是招手、`2` 是歪头、`3` 是伸懒腰、`4` 是眨眼睛。
- 右键菜单中的“设置...”会打开设置窗口，可修改启动宠物数量。当前上限为 5 只，设置会保存到系统用户配置目录。
